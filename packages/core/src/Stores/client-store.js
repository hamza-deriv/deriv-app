...
export default class ClientStore extends BaseStore {
    ...
    isEligibleForMoreDemoMt5Svg(market_type) {
        const isSynthetic = market_type === 'synthetic';
        const isAll = market_type === 'all';
        const isSwapFreeAccount = isAll ? 'all' : 'financial';
        ...
        const has_matching_account = this.trading_platform_available_accounts.some(account => {
            return (isSynthetic ? 'gaming' : isSwapFreeAccount) === account.market_type && account.shortcode === 'svg';
        });
        ...
        //         account =>
        //             (market_type === 'synthetic' ? 'gaming' : swapfree_account) === account.market_type &&
    ...
    isEligibleForMoreRealMt5(market_type) {
        ...
        // );
        // const swapfree = market_type === 'all' ? 'all' : 'financial';
        // const available_real_accounts_shortcodes = this.trading_platform_available_accounts
        //     .filter(
        //         account =>
        //             (market_type === 'synthetic' ? 'gaming' : swapfree) === account.market_type &&
        ...
        const isSynthetic = market_type === 'synthetic';
        const isAll = market_type === 'all';
        const isSwapFree = isAll ? 'all' : 'financial';
        ...
        const available_real_accounts_shortcodes = this.trading_platform_available_accounts
            .filter(
                account =>
                    (isSynthetic ? 'gaming' : isSwapFree) === account.market_type && account.shortcode !== 'maltainvest'
            )
            .map(account => account.shortcode);
